        ...
        public async Task<IActionResult> Invoice(int? id, bool sendEmail)
        {
            ...
            using(var stringWriter = new StringWriter())
            {
                ...
                if(sendEmail == true)
                {
                    ...
                    Response response = await _mailHelper.SendEmail(invoice.Customer.Email,"Invoice Pitstop Autoshop Lisbon",$"<h1>Service Invoice</h1>" +
        ...
        public async Task<IActionResult> Estimate(int? id, bool sendEmail)
        {
            ...
            using (var stringWriter = new StringWriter())
            {
                ...
                if (sendEmail == true)
                {
                    ...
                    Response response = await _mailHelper.SendEmail(estimate.Customer.Email, "Estimate Pitstop Autoshop Lisbon", $"<h1>Estimate</h1>" +
                        $" Mr/Mrs {estimate.Customer.FullName},<br>you can find attached to this email the estimate corresponding to the service done on " +
                        ...
                        $"<br> Thanks for trusting in our services! And hope to seen you soon! <br> Best regards, <br> Pitstop Autoshop Lisbon ", path);

                    if (response.IsSuccess == true)
                    {
                        _flashMessage.Confirmation($"Estimate was succefully sent to: {estimate.Customer.Email}.");
                        System.IO.File.Delete(path);
                        return RedirectToAction("Details", "Estimates", new { id = estimate.Id });
                    }
                    else
                    {
                        _flashMessage.Danger($"there was a problem sending the email to: {estimate.Customer.Email}.");
                        System.IO.File.Delete(path);
                        return RedirectToAction("Details", "Estimates", new { id = estimate.Id });
                    }
                }
